...
\label{thm:Connel}
    Let $G$ be a group. Then $\C[G]$ is prime
...
\label{cor:Connel}
    Let $G$ be a group. Then $\C[G]$ is left artinian if and only
    if $G$ is finite. 
\end{corollary}

\begin{proof}
    It follows from Theorem \ref{thm:Connel} and
    Hopkins--Levitzky theorem; see ... %\cite{} 
    for a proof. 
\end{proof}
...
%A nice application: Fermat's last theorem in finite rings.
