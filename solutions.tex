...
% Let $\omega$ be a primitive cubic root of one. 
% Consider the map 
% \[
% \varphi\colon \Q[G]\to \Q\times \Q\times M_2(\Q[\omega])
% \]
% given by $r\mapsto (1,1,R)$ and $s\mapsto (1,-1,S)$, where 
% \[
% R=\begin{pmatrix}
%     \omega & 0\\
%     0 & \omega^2
...
%     \end{pmatrix}. 
% \] 
% Since $R^3=S^2=\begin{pmatrix}1&0\\0&1\end{pmatrix}$ and $SRS=R^{-1}$, the map $\varphi$ 
% is a well-defined homomorphism. The map $\varphi$ is injective (prove it). The image of this
% map is way too big! Consider the map $\psi\colon\Q\times\Q\times M_2(\Q[\omega])$ which is
% the identity map on the first two components and is conjugation by 
% \[
% X=\begin{pmatrix}
%     1 & -\omega\\
%     1 & -\omega^2
% \end{pmatrix}
% \]
% in the third component. Then the composition 
% $\psi\varphi\colon \Q[G]\to \Q\times\Q\times M_2(\Q[\omega])$ is given by 
% \begin{align*} 
