...
\begin{definition}
	\index{Algebra}
	An \emph{$K$-algebra} (or an algebra over the field $K$) is a $K$-vector space $A$ 
...
%
%%  Finally, let $v\ne0$ and $w\in V$. Extend $v$ to a basis of $V$ 
%%  and define a $D$-linear map sending $v$ to $w$.
%%  Hence $\End_D(V)v=V$, proving simplicity.
%  It remains to show that $V$ is simple. It is enough to prove that
%  $V=\End_D(V)\cdot v=(v)$ for all $v\in V\setminus\{0\}$. Let $v\in
...
%    The vector space $M_n(\C)$ does not have a basis of nilpotent matrices. 
  ...
  \begin{proof}[Proof of Theorem \ref{thm:Wedderburn_nilpotent}]
    ...
    \]
    for all $\lambda_1,\lambda_2,\dots,\lambda_r\in K$. Thus $A^r=\{0\}$. 
