...
\begin{proof}
	...
	is either zero or an isomorphism with the image. Thus 
	\[
	M/N=\pi(M)=\sum_{i=1}^k(\pi|_{S_i})(S_i)
	\]
	is a direct sum of finitely many simples. 
	
	We now prove that $N$ is semisimple. 
	Since there exists a submodule $T$ such that 
	$M=N\oplus T$. The quotient
	$M/T$ is semisimple, so it follows that 
	\[
	N\simeq N/\{0\}=N/(N\cap T)\simeq (N\oplus T)/T=M/M
	\]
	is also semisimple.     
\end{proof}
