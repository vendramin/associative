...
if and only if 
there is a ring homomorphism $A\to Z(A)$, where $Z(A)=\{a\in A:ab=ba\text{ for all $b\in A$}\}$ is the center of $A$, 
such that $1_K\to 1_A$. 
...
Similarly, one defines \textbf{submodules} of $A$-modules. 
...
\begin{example}
If $M$ is a module over a $K$-algebra $A$, one defines $\End_A(M)$ as the set
