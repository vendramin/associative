...
\begin{proof}
   ...
   Let $V$ be a simple faithful module.
    ...
    Assume now that the theorem holds for all sets of size $\leq n$. 

    \begin{claim}
        There exist $\lambda_1,\dots,\lambda_n\in R$ such 
        that $\lambda_i\cdot x_i\ne 0$ for all $i\in\{1,\dots,n\}$ and 
        $\lambda_i\cdot x_j=0$ for all $i,j\in\{1,\dots,n\}$ with $i\ne j$. 
    \end{claim}
    ...
    $rx_i=z_i$ for all $i\in\{1,\dots,n-1\}$. Let 
    \[
...
\end{tikzcd}\]
    ...
    \]
    a contradiction to the linear independence of the set $\{x_1,\dots,x_{n-1}\}$. 
\end{proof}
