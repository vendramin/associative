...
\begin{proof}
    ...
    by Passman's lemma. Hence $K[\Delta(G)]$ has zero divisors, a contradiction
...
\begin{xca}
Let $G=\langle x,y:yxy^{-1}=x^{-1}\rangle$. Prove the following statements: 
\begin{enumerate}
    \item $x$ and $y$ are torsion-free. 
    \item $G$ is torsion-free. 
    \item $G\simeq \langle a,b:a^2=b^2\rangle$.
\end{enumerate}
\end{xca}
...
\[
	...
	\Longleftrightarrow
	\text{there exists $g\in G\setminus\{1\}$ such that $ga\in A$ and $g^{-1}a\in A$}.
\]
...
\begin{proof}
	...
	$C=AB$ admits at most one unique product. Then $|C|\geq2$. Since $G$ is diffuse, 
