...
\begin{proof}
    Let $a\in R$. For $n\geq0$, let
    \begin{align*}
    I_n&=(a,aX,aX^2,\dots,aX^n)\\
    &=R[X]a+R[X]aX+\cdots+aX^n+\Z a+\Z aX+\cdots+\Z aX^n.
    \end{align*}
    Then $I_0\subseteq I_1\subseteq\cdots I_n\subseteq I_{n+1}\subseteq\cdots$ 
    is a sequence of ideals of $R[X]$. Since $R[X]$ is noetherian,
    $I_n=I_{n+1}$ for some $n$. In particular, 
    $aX^{n+1}\in I_{n+1}=I_n$. Thus
    \[
    aX^{n+1}=\sum_{i=1}^n aX^{i-1}f_i(X)+\sum_{i=1}^n k_iaX^{i-1}
    \]
    for some $f_1(X),\dots,f_n(X)\in R[X]$ and 
    $k_1,\dots,f_n\in\Z$. Equating the coefficient of $X^{n+1}$ 
...
As an application, the polynomial ring $(2\Z)[X]$ is not 
...
\label{xca:I_fg}
    ...
    and $R$ be an $R$-module such that $I\cdot M=\{0\}$. Prove that
...
\begin{proof}
    ...
    Since $X\ne\empty$ and $R$ is artinian, there exists a minimal 
    element $I\in X$. The minimality of $I$ implies that 
    if $J$ is an ideal of $R$ such that $J\subseteq I$, then 
    $J$ is finitely generated. 

    \begin{claim}
        Either $R\cdot I=\{0\}$ or $R\cdot I=I$.
    \end{claim}
    
    If not, let $r\in R$ be such that $r\cdot I\ne\{0\}$ and $r\cdot I\ne I$. 
    Since $r\cdot I$ is an ideal of $R$ and
    $r\cdot I\subsetneq I$, the minimality of $I$ implies that 
    $r\cdot I$ is finitely generated. Let 
    $f\colon I\to r\cdot I$, $x\mapsto r\cdot x$. Then $f$ is a 
    surjective module homomorphism. Since $R\cdot I\ne\{0\}$, 
    $f$ is non-zero. In particular, $\ker f\subsetneq I$ 
    is finitely generated, again by the minimality of $I$. 
    By the first isomorphism theorem, $I/K\simeq r\cdot I$ as $R$-modules.
    Since $\ker f$ and $I/\ker f\simeq r\cdot K$ are finitely generated, 
    $I$ is finitely generated, a contradiction.
    
    \begin{claim}
        $M=\{r\in R:r\cdot I=\{0\}\}$ is a maximal ideal of $R$. 
    \end{claim}
    
    Routine calculations show that $M$ is an ideal. Since 
    $R$ is artinian, it is enough to show that $M$ is a prime ideal. 
    Let $rs\in M$. Then $(rs)\cdot I=\{0\}$. If $r\cdot M$, 
    then $r\cdot I\ne\{0\}$. By the previous claim, $r\cdot I=I$. Thus
    \[
    \{0\}=(rs)\cdot I=s\cdot (r\cdot I)=s\cdot I
    \]
    and hence $s\in M$.     
    
    \medskip
    Since $M$ is maximal, $K=R/M$ is a field. 
    Since $M\cdot I=\{0\}$, $I$ is an $(R/M)$-module, that is 
    ...
    $B\setminus\{0\}$. Then $J$ is an ideal of $R$, 
    as  
    \[
    J=\sum_{x\in B\setminus\{x_0\}} R\cdot x. 
    \]
