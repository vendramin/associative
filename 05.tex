...
Let $K$ be a field, and $G$ be a group. The \emph{group algebra}\index{Group algebra} $K[G]$ 
...
\begin{exercise}
	...
	The \emph{support}\index{Support!of a group algebra element} of $\alpha$ is the set 
	\[
...
\begin{theorem}
    Let $G$ be a finite group. The number of isomorphism classes of simple modules of $\C[G]$
...
\label{exa:C4}
    ...
    $G$ has four isomorphism classes of simple modules and 
    $\C[G]\simeq\C^4$. 
\end{example}

\begin{example}
\label{exa:S3}
    Let $G=\Sym_3$. Then $G$ has three isomorphism classes of simple modules and
    \[
...
Let $R$ be a unitary commutative ring and $G$ be a group. Recall that the \emph{group ring}\index{Group ring} $R[G]$ is a free $R$-module which is also a ring. As a free
...
\]
where all but finitely many coefficients are zero. 
...
\label{question:iso}
\index{Isomorphism problem!for group rings}
...
\begin{que}
\label{question:IP}
\index{Isomorphism problem!integral}
...
\index{Isomorphism problem!modular}
\label{question:MIP}
...
\begin{question}
\label{Jacobson semisimplicity problem}
Let $G$ be a group and $K$ be a field. When does $J(K[G])=\{0\}$ hold?
\end{question}

As an application of Amitsur's theorem \ref{thm:Amitsur}, 
we prove that 
complex group algebras have zero Jacobson radical.
This is known as 
...
\begin{proof}
	We first prove that $1)\implies2)$. If $I^2=\{0\}$ and $x\in I$, then
...
\begin{graybox}
  ...
  For a group $G$, one defines the \emph{trace}\index{Trace!of a group algebra element} 
  of an element 
  \[
  ...
  \begin{proof}
	...
	Assume that $J(\C[G])\ne\{0\}$. Choose
	$\alpha\in J(\C[G])$ as in Lemma~\ref{lem:algebraico}, so that
	$\trace(\alpha^{2^m})\geq1$ for every $m\geq1$.
	Let $\varphi(z)=(1-z\alpha)^{-1}$ and define
	%$f(z)=\trace(\varphi(z))$.
    ...
    % $\trace(\alpha^{2^m})\geq1$ for all $m\geq0$. 
	%This contradicts the limit computed in~\eqref{eq:limite}. Hence $\alpha=0$.
	% This contradicts $\trace(\alpha^{2^m})\geq1$ for every $m\geq1$.
	% Therefore $J(\C[G])=\{0\}$.
	The limit in~\eqref{eq:limite} contradicts the choice of
	$\alpha$, since $\trace(\alpha^{2^m})\geq1$ for every $m\geq1$.
	Therefore $J(\C[G])=\{0\}$.
  \end{proof}
  ...
  \begin{proof}
    ...
    M=JM$ is an $S$-submodule of $M$, as 
    \[
...
\begin{proof}
	...
	Conversely, assume that $K$ has characteristic $p>0$ and that $p$ divides $n$.
	Let $\alpha=\sum_{i=1}^ng_i$. Since $\alpha
	...
	\]
	in $K[G]$, it follows that $I$ is a nilpotent non-zero ideal. Thus $J(K[G])\ne\{0\}$, 
...
\begin{corollary}
	...
	and suppose that the characteristic
  	of $K$ is zero or does not divide $|G|$. 
    Then $K[G]$ does not contain non-zero nil left ideals. 
\end{corollary}
...
Our aim now is to answer the following question: When is 
a group algebra algebraic? Herstein's theorem provides
...
\begin{example}
\index{Prufer group@Pr\"ufer group}
	Let $p$ be a prime number. 
	The \emph{Pr\"ufer group}  
	\[
...
\begin{example}
    Abelian torsion groups are locally finite. Let $G$ be an abelian torsion group
...
\begin{proof}
	Let $\pi\colon G\to G/N$ be the canonical map and $\{g_1,\dots,g_n\}$ be a finite subset of $G$. Without loss of generality, we may assume that $1\in\{g_1,\dots,g_n\}$. Let $H$ be the subgroup of $G$ generated
	by $\{g_1,\dots,g_n\}$. 
	...
	For each $i,j\in\{1,\dots,m\}$ choose $u_{ij}\in N$ and 
	$k\in\{1,\dots,m\}$ such that \[
	g_ig_j=u_{ij}g_k.
	\]
	Let $U$ be the subgroup of $N$
	generated by $\{u_{ij}:1\leq i,j\leq m\}$. Since $N$ is locally finite, $U$ is finite.
	By induction on word
	length, every positive word in $g_1,\dots,g_n$ has the form $ug_k$, with $u\in U$ and $1\leq k\leq m$: if a word has this form, then
	\[
 	(ug_k)g_j=u u_{kj}g_l.
	\]
	Let $S$ be the set of positive words in $g_1,\dots,g_n$.
	Then $S\subseteq\bigcup_{k=1}^m Ug_k$, so $S$ is finite.
	Since a finite submonoid of a group is a subgroup, $S=H$.
	Thus $H\subseteq\bigcup_{k=1}^m Ug_k$, and
	$|H|\leq m|U|<\infty$.	
	% Moreover, since 
...
\begin{proof}
	...
	The coefficient of $h\in H$ in $\alpha^k$ is the number
	of words of length $k$ in $x_1,\dots,x_m$ that represent $h$.
	In characteristic zero, this coefficient is non-zero whenever
...
The \emph{tensor product}\index{Tensor product}\index{Tensor product!of vector spaces} of the vector spaces (over $K$) $U$ and $V$ 
...
\begin{proof}
	...
	\]
	is bilinear. From the definitions, it follows that every element of 
	...
	\end{tikzcd}
	\]
	commutes. In particular, $\overline{\beta}(u\otimes v)=\beta(u,v)$. Uniqueness follows because pure tensors span $U\otimes V$. 
\end{proof}
...
\begin{proof}
	...
	\]
	is well-defined. Uniqueness follows because the pure tensors span $U\otimes V$.
\end{proof}
...
\begin{proposition}
	...
	$v_1,\dots,v_n\in V$ are such that $\sum_{i=1}^n u_i\otimes v_i=0$, then 
    $v_i=0$ for all $i\in\{1,\dots,n\}$.
\end{proposition}

\begin{proof}
	Extend $u_1,\dots,u_n$ to a basis of $U$. For each $i$, let $f_i\colon U\to K$ be the linear map satisfying 
	\[
	...
	\end{cases}
	\]
	and vanishing on the additional basis vectors. 
	Since the map 
 	\[
...
\begin{proof}
	...
	\]
	Uniqueness follows from Proposition~\ref{pro:U_LI}, applied
	to the difference of two such expressions.
\end{proof}
...
\index{Tensor product!of algebras}
...
\begin{proof}
	...
	$A\otimes K[X]\to A[X]$, $\sum a_i\otimes X^i\mapsto \sum a_iX^i$, is a 
...
These are examples of tensor-product constructions.
A particularly useful case is extension of the base field; the procedure is called 
\emph{scalar extension}. 
...
\begin{exercise}
    Prove the following statements:
    \begin{enumerate}
		\item $\{1\otimes a:a\in A\}$ 
		%$\{1\}\otimes A$ 
		is a $K$-subalgebra of $A^E$ isomorphic to $A$.
		\item If $\{a_i:i\in I\}$ is a $K$-basis of $A$, then $\{1\otimes a_i:i\in
			I\}$ is an $E$-basis of $A^E$.
	\end{enumerate}
\end{exercise}
...
Combining extension of scalars with Formanek's theorem for rational group algebras yields the following result.
...
\begin{proof}
	...
	K\otimes_{\Q}\Q[G]$. 
	% $Note that each $\beta\in K\otimes_{\Q}\Q[G]$ can be written
	% uniquely as 
	% \[
	% 	\beta=1\otimes\beta_0+\sum k_i\otimes\beta_i,
	% \]
	% where $\{1,k_1,k_2,\dots,\}$ is a basis of $$K$ as a $\Q$-vector space. 
	Choose a $\Q$-basis $\{1\}\cup\{k_i:i\in I\}$ of $K$.
	Every $\beta\in K\otimes_{\Q}\Q[G]$ can be written uniquely as
	\[
  	\beta=1\otimes\beta_0+\sum_{i\in I}k_i\otimes\beta_i,
	\]
	where $\beta_0,\beta_i\in\Q[G]$ and all but finitely many
	$\beta_i$ are zero.
    ...
    Let $\beta\in K[G]\simeq K\otimes_{\Q}\Q[G]$ be such that $\alpha\beta=1$. Since
	\[
	1\otimes 1=(1\otimes\alpha)\beta=1\otimes \alpha\beta_0+\sum k_i\otimes \alpha\beta_i,
	\]
it follows that $\alpha\beta_0=1$.
Since $\alpha$ is invertible in $K[G]$, this equality implies
that $\beta_0=\alpha^{-1}$. Thus $\alpha$ is invertible in $\Q[G]$.

Now suppose that $\alpha$ is a zero divisor in $K[G]$.
If $\alpha\beta=0$ for some $0\ne\beta\in K[G]$, then
coefficient comparison gives
\[
  \alpha\beta_0=0,\qquad
  \alpha\beta_i=0\quad\text{for every }i\in I.
\]
Since $\beta\ne0$, at least one of $\beta_0$ and the $\beta_i$
is non-zero. Hence $\alpha$ is a zero divisor in $\Q[G]$.
The case $\beta\alpha=0$ for some $0\ne\beta\in K[G]$
is analogous.

Therefore every element of $\Q[G]$ is invertible or a zero
divisor. Theorem~\ref{thm:FormanekQ} implies that $G$ is
locally finite.
	% it follows that $\alpha\beta_0=1$. 
	% Since $\alpha$ is invertible in $K[G]$, the equality $\alpha\beta_0=1$ implies that $\beta_0=\alpha^{-1}$.
	% Thus $\alpha$ is invertible in $\Q[G]$.
	% Similarly, if
	...
	% for $\Q$ applies.
	% If $\alpha\beta=0$ for some $0\ne\beta\in K[G]$, then
 	% $\alpha\beta_j=0$ for every coefficient $\beta_j$.
 	% Since $\beta\ne0$, at least one $\beta_j$ is non-zero,
 	% so $\alpha$ is a zero divisor in $\Q[G]$.
 	% The argument for $\beta\alpha=0$ is analogous.
\end{proof}
