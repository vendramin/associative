...
\begin{proof}
    For clarity, we will prove the result for $n = 3$. 
    The extension to general $n$ is straightforward and left as an exercise for the reader.
    
    We first prove that $J(M_3(R))\subseteq M_3(J(R))$. 
	If $J(R)=R$, the theorem is clear. Let us assume that $J(R)\ne R$ and let  
	$J=J(R)$. 
	If $M$ is a simple $R$-module, then $M^3$ is a simple $M_3(R)$-module with the usual multiplication. 
	...
	\[
		...
		\end{pmatrix}\in J_2.
	\]
	Since $x_2$ is quasi-regular, there exists $y_2\in R$ such that $x_2+y_2+x_2y_2=0$.
	For 
	\[
		y=\begin{pmatrix}
			0 & 0 & 0\\
	...
	\begin{align*}
		x+(y-u-yu)+x(y-u-uy)=0,
	\end{align*}
	and therefore $J_2$ is right quasi-regular. Similarly one proves that 
	both $J_1$ and $J_3$ are right quasi-regular. Thus $J_i\subseteq J(M_3(R))$ for all 
	$i\in\{1,2,3\}$. In conclusion, 
	\[
...
Open problem~\ref{prob:Koethe} is the well-known \emph{K\"othe's conjecture}. 
