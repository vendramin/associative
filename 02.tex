...
\begin{proof}
	Since $A$ is simple, it follows that $A$ is semisimple. The Artin--Wedderburn theorem implies that $A\simeq\prod_{i=1}^k M_{n_i}(D_i)$ 
	for some $n_1,\dots,n_k$ and division algebras $D_1,\dots,D_k$. Moreover, $A$ has 
	$k$ isomorphism classes of simple modules. Since $A$ is simple,
	$A$ has only one isomorphism class of simple modules (Proposition~\ref{pro:unique_simple}). Thus $k=1$ and hence 
