...
\begin{proof}
   ...
   Let $V$ be a simple faithful module.
    ...
    Let $n\geq1$ and assume that the theorem holds for all sets of size $\leq n-1$. 
...
\begin{proof}
    ...
    imply that $R=\End_D(V)\simeq M_n(D)$, as $\dim_DV<\infty$. 
...
\begin{proof}
    Since $I$ is primitive, then $R/I$ is primitive (Exercise~\ref{xca:primitivo}). By Wedderburn theorem, 
...
\begin{proof}[Proof of Theorem \ref{thm:ArtinWedderburn_version2}]
    ...
    \end{align*}
    From this, it follows that $\varphi$ is surjective: if $(x_1+I_1,\dots,x_k+I_k)\in\prod_{j=1}^kR/I_j$, then
    \begin{align*}
...
\begin{proof}
    ...
    it follows that $x_1\in I\cap L_1$. Thus $I\cap L_1$ is a non-zero left ideal contained in the minimal left ideal $L_1$, a contradiction. Now the claim follows, as
