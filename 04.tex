...
\section{Semiprimitive rings}

\begin{definition}
	\index{Ring!semiprimitive}
	\index{Jacobson semisimple ring|see{Ring, semiprimitive}}
...
We proved in Theorem~\ref{thm:J(R/J)=0} 
that $R/J(R)$ is semiprimitive. 
...
%	$\prod_{i\in I}R_i$ is said to be a \emph{subdirect product} of %the
...
\begin{example}
    ...
    r\mapsto (r+I_i)_i.
    \]
	The canonical map $f$ exhibits $R$ as a subdirect product
	of the $R/I_i$ if and only if $f$ is injective, equivalently
	if and only if $\bigcap_i I_i=\{0\}$.		
    % For each
...
\section{Jacobson's density theorem}

At this point, it is convenient to recall that unitary 
modules over division rings behave much like vector spaces over fields. For that reason, 
they are often called \emph{vector spaces over division rings}. 

\begin{definition}
	\index{Dense ring of linear operators}
	Let $D$ be a division ring, and $V$ be a non-zero vector space over $D$. A subring 
	$R\subseteq\End_D(V)$ is a \emph{dense ring of linear operators} 
	of $V$ (or simply, \emph{dense} in $V$) if for every  
...
\begin{proposition}
    ...
    $V$ be a non-zero finite-dimensional $D$-vector space.  
...
\begin{theorem}[Jacobson]
	...
	A unitary ring is primitive if and only if it is isomorphic to a dense ring on 
...
\begin{proof}
   ...
   Assume now that $R$ is a primitive ring. 
   ...
   Let $V$ be a simple faithful module.
    ...
    Let $n\geq2$ and assume that the theorem holds for all sets of size $\leq n-1$. 
    Let $\{x_1,\dots,x_{n}\}$ be a linearly independent subset of $V$ and 
    $\{y_1,\dots,y_{n}\}$ be a subset of $V$. 
    ...
    It remains to prove the claim. To prove it, suppose that it is not true. After relabeling the $x_i$, 
	we may assume that 
	$\lambda x_i=0$ for all $i\in\{1,\dots,n-1\}$ implies  
    $\lambda x_n=0$. By the inductive hypothesis, given $z_1,\dots,z_{n-1}\in V$, 
    ...
    % https://q.uiver.app/#q=WzAsMyxbMCwwLCJNIl0sWzEsMCwiTV57bi0xfSJdLFsyLDAsIk0iXSxbMCwxLCJcXGlvdGFfaSJdLFsxLDIsIlxccGhpIl1d
\[\begin{tikzcd}
...
\begin{bonus}
    Prove Jacobson's density theorem for non-unitary rings. 
\end{bonus}
...
\begin{corollary}
	If $R$ is a primitive ring, there exists a division ring $D$ such that either 
	$R\simeq\End_D(V)$ for some finite-dimensional vector space $V$ over $D$ or 
...
For example, the ring $M_n(\C)$ is not a domain for $n\geq2$.
We need a non-commutative generalization of domains.

\begin{definition}
	\index{Ring!prime} 
	Let $R$ be a non-zero ring, not necessarily with identity. Then $R$ is
	\emph{prime} if for $x,y\in R$ such that $xRy=\{0\}$ it follows that $x=0$ or 
	$y=0$.
\end{definition}

\index{Domain}
A non-zero ring $R$ is a \emph{domain} if $xy=0$ implies
$x=0$ or $y=0$. Each domain is trivially a prime ring.

\begin{example}
    A commutative ring is prime if and only if it is a domain. If $R$ is commutative and prime, 
	then $ab=0$ implies $aRb=\{0\}$, so $a=0$ or $b=0$. Conversely, every domain is prime. 
\end{example}
...
\begin{proof}
	...
	\{0\}$, $u\in I$ and $v\in J\setminus\{0\}$, then $uRv\subseteq IRJ=\{0\}$. Hence 
	$u=0$.

	The implication $2)\implies3)$ is trivial. 

    Now we prove that $3)\implies1)$. Let $x,y\in R$ be such that $xRy=\{0\}$, and 
...
\begin{example}
	If $R_1$ and $R_2$ are non-zero rings, $R=R_1\times R_2$ is not prime, as 
...
We studied minimal left ideals of rings before (see Definition~\ref{def:minimal_left_ideal}).  
...
\begin{proof}
	Let $y\in L\setminus\{0\}$. Then $0\ne yRy\subseteq RL$. Since $L$ is a minimal left ideal, 
	it follows that $L$ is a simple $R$-module.  
	We claim that $L$ is faithful. Let $y\in L\setminus\{0\}$ and
	$x\in\Ann_R(L)$. Since $xRy\subseteq xRL\subseteq xL=\{0\}$, it follows that 
	$x=0$.
\end{proof}
...
\begin{proof}
	...
	$V$ is a module over $R$ with $f\cdot v=f(v)$, where $f\in R$ and $v\in V$. 
	For $n\in\Z_{>0}$ let 
	\[
...
\begin{theorem}[Wedderburn]
	\index{Wedderburn's theorem!simple artinian rings}
	Let $R$ be a left artinian ring. The following statements are equivalent:
	\begin{enumerate}
		\item $R$ is simple.
		\item $R$ is prime.
		\item $R$ is primitive.
		\item $R\simeq M_n(D)$ for some $n\geq1$ and some division ring $D$.
	\end{enumerate}
\end{theorem}
...
\begin{proof}
	...
	Now Lemma~\ref{lem:denso_artiniano} and Proposition~\ref{pro:unique_dense} 
    imply that $R=\End_D(V)\simeq M_n(D^{\op})$. Since 
	$D^{\op}$ is a division ring, the claim follows. 
	% as $\dim_DV<\infty$. 

	Finally, $4)\implies1)$ is trivial, as $M_n(D)$ is simple. 
\end{proof}

\section{The Artin--Wedderburn theorem for rings}

We now prove the Artin--Wedderburn theorem. We will assume that our ring
is a unitary left artinian ring. One could prove the 
theorem for arbitrary rings --see for example \cite{MR600654}--  
...
implies that the map is in fact surjective. 

\begin{definition}
\index{Ideal!prime}
    A proper ideal $I$ of $R$ is \emph{prime} if $xRy\subseteq I$ implies
    $x\in I$ or $y\in I$.
\end{definition}
...
\begin{proof}
    Since $I$ is primitive, $R/I$ is primitive (Exercise~\ref{xca:primitivo}). By Wedderburn's theorem, 
...
\begin{proof}
    ...
    by Exercise~\ref{xca:primitivo}. By Wedderburn's theorem, $R/I$ is
    simple. Thus $I$ is maximal by the correspondence theorem. % before: proposition:R/I
\end{proof}

\begin{lemma}
\label{lem:I2...In}
    Let $R$ be a left artinian unitary ring.
    For $k\geq2$, let $I_1,\dots,I_k$ be finitely many distinct maximal ideals of $R$. 
    Then $I_2\cdots I_k\not\subseteq I_1$.   
\end{lemma}
...
\begin{proof}
    ...
    $I_k\ne I_1$. 
    Then 
    $(xR)y\subseteq II_k\subseteq I_1$. Since $I_1$ is prime (this is left as an exercise), 
    it follows that either $x\in I_1$ or $y\in I_1$, a contradiction.   
\end{proof}

\begin{exercise}
    Complete the proof of Lemma~\ref{lem:I2...In}. 
\end{exercise}

\begin{lemma}
\label{lem:Rartinian_finite_primitives}
    Let $R$ be a left artinian unitary ring. Then $R$ has only 
    finitely many primitive ideals.
\end{lemma}

\begin{proof}
    Assume that $I_1,I_2,\dots$ are infinitely many distinct primitive ideals. 
...
\begin{proof}[Proof of Theorem \ref{thm:ArtinWedderburn_version2}]
	If $R=\{0\}$, then the result holds with the empty product. So assume that $R\ne\{0\}$. 
    Let $I_1,\dots,I_k$ be the (distinct) primitive ideals of $R$. If $k=1$, then $I_1=J(R)=\{0\}$ and 
	the theorem holds. So assume that $k\geq2$. 
    Each $I_i$ is a maximal ideal. Thus $I_i+I_j=R$ for
    ...
    \[
    x_1\in (1+I_1)\cap (I_2\cdots I_k)\subseteq I_j
    \]
    ...
    \]
    Similarly,
    there exist $x_2\in 1+I_2,\dots, x_k\in 1+I_k$ such that 
    \begin{align*}
    \varphi(x_2)&=(I_1,x_2+I_2,\dots,I_k),\\
    &\vdots\\
    \varphi(x_k)&=(I_1,I_2,\dots,x_k+I_k).
    \end{align*}
    From this, it follows that $\varphi$ is surjective: 
	let $(a_1+I_1,\dots,a_k+I_k)\in\prod_{j=1}^kR/I_j$.  
	Then
	\[
  		\varphi\left(\sum_{i=1}^k a_i x_i\right)
  		=(a_1+I_1,\dots,a_k+I_k).
	\]
	Thus $\varphi$ is surjective.
	% if $(x_1+I_1,\dots,x_k+I_k)\in\prod_{j=1}^kR/I_j$, then
    % \begin{align*}
    %     \varphi(x_1+\cdots+x_k) &= \varphi(x_1)+\cdots+\varphi(x_k)\\
    %     &=(x_1+I_1,I_2,\dots,I_k)+\cdots+(I_1,I_2,\dots,x_k+I_k)\\
    %     &=(x_1+I_1,\dots,x_k+I_k).
    % \end{align*}
    
    Each $R/I_i$ 
    is primitive and hence isomorphic to $M_{n_i}(D_i)$ for some 
    $n_i$ and some division ring $D_i$ (by Wedderburn's theorem). Therefore
    \[
...
% An $R$-module is \emph{semisimple} if it is isomorphic to
% a direct sum of simple modules. A finitely generated
% semisimple module is a finite direct sum of simple modules.
...
\begin{proof}
	Assume that $R\ne\{0\}$, otherwise the result is trivial. 
...
that every semisimple ring is both left artinian and left noetherian. 

\begin{exercise}
\label{xca:semisimple}
    If $R$ is a semisimple ring, every unitary $R$-module is semisimple. 
\end{exercise}
...
\end{pmatrix}\right\}.
\]
Both these column spaces are indeed minimal left ideals of $M_2(\R)$. 
...
\begin{proof}
	If $R=\{0\}$, the result is trivial. So assume that $R\ne\{0\}$. 
    ...
    that $M_1$ is maximal. If not, there exists a proper left ideal $I$ of $R$ 
    such that $M_1\subsetneq I$. Let $x\in I\setminus M_1$ and write 
    \[
    x=x_1+x_2+\cdots+x_k
    \]
    for $x_j\in L_j$. Since $x_2+\cdots+x_k\in M_1\subseteq I$, 
    it follows that $x_1\in I\cap L_1$. Since $x\notin M_1$, we have $x_1\ne0$. Thus $I\cap L_1$
	is non-zero, and the minimality of $L_1$ gives $I\cap L_1=L_1$.
	Hence $R=M_1+L_1\subseteq I$, contradicting the fact that
	$I$ is proper.
	%Thus $I\cap L_1$ is a non-zero left ideal contained in the 
	%minimal left ideal $L_1$, a contradiction. 
    ...
    for division rings $D_1,\dots,D_k$, this is the 
...
\section{The Hopkins--Levitzki theorem}

\begin{theorem}[Hopkins--Levitzki]
	\label{thm:Hopkins-Levitzki}
	\index{Hopkins--Levitzki theorem}
...
\begin{proof}
	...
	by Theorem~\ref{thm:Jnilpotente}. Let $n$ be such that $J^n=\{0\}$. 
	...
	% For $0\leq i<n$, the module $J^i/J^{i+1}$ is a unitary
	% $(R/J)$-module, so it is semisimple. It is also artinian
	% as an $R$-module, being a subquotient of the artinian
	% module $\prescript{}{R}R$. Since its $R$-submodules and
	% $(R/J)$-submodules coincide, it is artinian as an
	% $(R/J)$-module. An artinian semisimple module is a finite
	% direct sum of simple modules. Hence each $J^i/J^{i+1}$
	% is noetherian as an $R$-module.
	In particular, since every unitary $(R/J)$-module is semisimple,  
	each $J^{i}/J^{i+1}$ 
	is semisimple. Moreover, each $J^{i}/J^{i+1}$ is 
	artinian (because it is a subquotient of $\prescript{}{R}R$). 
	An artinian semisimple module is a finite
	direct sum of simple modules, and hence each 
	$J^{i}/J^{i+1}$ is noetherian as an $R$-module.  
