...
we now present the converse of Hilbert's basis theorem\index{Hilbert's basis theorem}. 
...
\begin{proof}
    ...
    \end{align*}
    Then $I_0\subseteq I_1\subseteq\cdots\subseteq I_m\subseteq I_{m+1}\subseteq\cdots$ 
    ...
    \]
    for some $f_1(X),\dots,f_{n+1}(X)\in R[X]$ and 
    $k_1,\dots,k_{n+1}\in\Z$. Comparing the coefficient of $X^{n+1}$ 
    ...
    $R$ is noetherian. In particular, $R$ is finitely generated as an ideal of itself, 
    say 
    \[
...
\label{pro:artinian_minimal}
	...
	every non-empty set of submodules of $N$ 
...
Recall that a ring $R$ is \emph{left noetherian}\index{Ring!left noetherian}
if its left regular module $\prescript{}{R}R$ is noetherian.
...
Similarly one defines right artinian rings\index{Right!right artinian}. 
...
\begin{proof}
	...
	$J^k\ne\{0\}$ let $\mathcal{S}$ be the set of left ideals 
	$I$ such that $J^kI\ne\{0\}$. Since 
	\[
	J^kJ^k=J^{2k}=J^k\ne\{0\},
	\]
...
In Definition~\ref{def:serie_de_composicion}, we allow $n=0$: the zero module has the one-term composition series $\{0\}$, of length zero. 

The definition also applies to modules over non-unitary rings.
We require each factor to be non-zero with no non-zero proper
submodules, but do not require the action of $R$ on that factor to be non-zero.
...
\begin{proof}
	If $M=\{0\}$, both series have length zero and the result is trivial.
	...
	$M_{i+1}/M_{i}$ has no non-zero proper submodules and the sequence 
	\[
	\begin{tikzcd}
		0 \arrow{r}
		& M_i \arrow{r}
	...
	\]
	of submodules of $M$, where each $M_{i+1}/M_i$ is non-zero and admits no non-zero
	proper submodules. Since $M$ is noetherian, this process must terminate, and it can
	terminate only when $M_n=M$. 
\end{proof}
...
\index{Composition series!equivalence}
	...
	$V_{i-1}/V_{i}\simeq W_{\sigma(i)-1}/W_{\sigma(i)}$
...
\begin{proof}
	...
	in this case $M$ has no non-zero proper submodules and $M\supsetneq\{0\}$ 
	...
	If $U=\{0\}$, take its composition series of length $r=0$.
	Otherwise, by Theorem~\ref{thm:serie_de_composicion}, 
	$U$ admits a composition series 
	\[
		U=U_0\supsetneq U_1\supsetneq\cdots\supsetneq U_r=\{0\}.
	\]
    Thus
    $V_1\supsetneq\cdots\supsetneq V_k=\{0\}$ and  
	$V_1\supsetneq U\supsetneq U_1\supsetneq\cdots\supsetneq U_r=\{0\}$ are both composition 
	...
	Therefore $l=k$. The two original series have, up to
	isomorphism and permutation, the factors $M/V_1$, $M/W_1$,
	and $U_{j-1}/U_j$ for $1\le j\le r$. Hence they are equivalent.
\end{proof}

The Jordan--H\"older theorem allows us to define the 
...
\index{Module!finite length}
A module is said to be of 
\emph{finite length} if it admits a composition series. 
...
\begin{exercise}
  \label{xca:I_fg}
  Let $R$ be a unitary commutative ring, let $I$ be an ideal of $R$
  and let $M$ be an $R$-module such that $I\cdot M=\{0\}$. Prove the following statements:
  \begin{enumerate}
    \item If $M$ is finitely generated, then $M$ is a finitely generated
      $(R/I)$-module with
      \[
        (r+I)\cdot m=r\cdot m,\quad r\in R,m\in M.
      \]
    \item $M$ is finitely generated as an $R$-module
      if and only if it is finitely generated as an $(R/I)$-module.
  \end{enumerate}
\end{exercise}

Recall that a proper ideal $I$  
of a commutative ring 
$R$ is said to be \emph{prime}\index{Ideal!prime} if 
$xy\in I$ implies that $x\in I$ or $y\in I$. 

\begin{exercise}
    Let $R$ be a unitary commutative artinian ring. 
    \begin{enumerate}
...
\begin{proof}
    ...
    \begin{claim}
        %Either $RI=\{0\}$ or $RI=I$.
		For every $r\in R$, either $rI=\{0\}$ or $rI=I$.
    \end{claim}
    ...
    Routine calculations show that $M$ is an ideal. 
	Moreover, $M$ is proper, as $I\ne\{0\}$ and $1I=I$.
	Since 
