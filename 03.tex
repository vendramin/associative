...
This definition also applies to modules over non-unitary rings.
We require each factor to be non-zero with no non-zero proper
submodules, but do not require the action of $R$ on that factor
to be non-zero.
...
\begin{proof}
	...
	$M_i/M_{i-1}$ has no non-zero proper submodules and the sequence 
	\[
	\begin{tikzcd}
		0 \arrow{r}
		& M_i \arrow{r}
